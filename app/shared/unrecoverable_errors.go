...
func GetUnrecoverableEdgeCases() []UnrecoverableEdgeCase {
	return []UnrecoverableEdgeCase{
		...
		},
		{
			...
			Description: "The original file content snapshot needed for rollback is missing.",
			Scenarios: []string{
				"Snapshot pruned to save space",
				"Crash before snapshot completed",
				"Storage failure",
			},
			WhyUnrecoverable: "Rollback requires the original file content. Without the snapshot, the system cannot restore the file.",
			DataAtRisk:       "Original file content cannot be recovered through the transaction system",
			PartialRecovery: &PartialRecoveryOption{
				Possible:    true,
				Description: "Recover from git or external backup",
				Steps: []string{
					"Check git for file history",
			...
			Prevention: []string{
				"Always commit before plandex operations",
				"Configure snapshot retention policy",
				"Enable redundant snapshot storage",
			},
		},
		{
